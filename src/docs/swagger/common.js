 ...
 *     description: >
 *       Sends a test FCM push notification using Firebase Admin SDK (`src/config/fada-id-firebase-key.json`).
 *       Provide `token` or `tokens`. By default the notification is queued (requires `npm run worker`).
 *       Set `sync: true` to send immediately without the queue.
 *       Requires `IS_PUSH_ENABLED=true`.
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
 ...
 *       422:
 *         description: title, body, and token/tokens are required
 *       500:
